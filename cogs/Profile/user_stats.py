...
class UserStats(commands.Cog):
    ...
    @app_commands.command(name="user_stats", description="...")
    @app_commands.checks.cooldown(2, 10, key=lambda i: i.user.id)
    async def user_stats(self, interaction: discord.Interaction, user_id: str = None):

        async def fetch_and_show_stats(inter, role, canonical_id):
        ...
        async def premium_stats_callback(inter, role, identifier, view):
            ...
            async with session.post(resolve_url, json=packet.to_dict(), headers=headers) as resp:
                    if resp.status == 200:
                        res = await resp.json()
                        # Increment view count on backend
                        await increment_profile_view(inter.user.id, res['role'], res['canonical_id'])
                        embed = await fetch_and_show_stats(inter, res['role'], res['canonical_id'])
        ...
        await validate_and_respond(interaction, stats_callback)

    def build_stats_embed(self, data, role):
        ...
        exp = stats.get('exp', 0)
        experience_level = stats.get('experience_level', 'N/A').title()
        profile_views = stats.get('profile_views', 0)
        jobs_finished = stats.get('jobs_finished', 0)
