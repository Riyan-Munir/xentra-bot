...
class JobMessageConfirmView(discord.ui.View):
    ...
    async def add_files(
        ...
        upload_info = (
            ...
            '`1.` Drag & drop your files here or use the Discord attachment button.\n'
            f'`2.` Allowed: **{", ".join(sorted(ALLOWED_EXTENSIONS))}**\n'
            f'`3.` Max **{MAX_ATTACHMENTS}** files · Under **{MAX_TOTAL_SIZE_MB} MB** total\n'
            '\n'
            '> __Waiting for files — upload them now.__'
        )
        instruction_embed = info_embed(message=upload_info, footer='Xentra • Rooms')
        try:
            await self.original_interaction.edit_original_response(
...
class JobMessage(commands.Cog):
    ...
    async def job_message(
        ...
        async def callback(user_data: dict):
            ...
            desc_parts.append(
                '\n> __Click Write Message to compose your message, or Cancel to abort.__'
            )
