"""
``/interview delivery`` and ``/job delivery``, Retry any failed system
message deliveries (rules, job details) for your active rooms.

The bot stores a backend record when it cannot reach a user (DMs disabled /
blocked).  These commands fetch all unresolved records for the calling user,
re-attempt delivery via ``handle_system_message()``, and mark successes as
resolved.
"""
...
class InterviewDelivery(commands.Cog):
    ...
    async def _run_retry(
        ...
        """Fetch pending deliveries for *room_type* and re-attempt them."""
        ...
        if not pending:
            return success_embed(
                message='All room messages delivered successfully.',
            )
    ...
    async def interview_delivery(self, interaction: discord.Interaction) -> None:
        """Fetch pending interview deliveries, attempt re-delivery, show results."""

        async def callback(user_data: dict) -> discord.Embed:
            return await self._run_retry(interaction, 'interview')
    ...
    @app_commands.checks.cooldown(1, 30, key=lambda i: i.user.id)
    async def job_delivery(self, interaction: discord.Interaction) -> None:
        """Fetch pending job deliveries, attempt re-delivery, show results."""

        async def callback(user_data: dict) -> discord.Embed:
            return await self._run_retry(interaction, 'job')
