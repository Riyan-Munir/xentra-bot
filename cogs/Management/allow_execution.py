...
logger = logging.getLogger('bot.allow_execution')
...
class AllowExecutionView(discord.ui.View):
    def __init__(self, identifier, callback_func, user_data):
        ...
        self._done = False
        self.add_item(AllowExecutionSelect(identifier))
...
class AllowExecution(commands.Cog):
    """``/allow_execution``, Grant bot command execution permission to a user."""
    ...
    @app_commands.command(name="allow_execution", description="...")
    @app_commands.checks.cooldown(1, 10, key=lambda i: i.user.id)
    async def allow_execution(self, interaction: discord.Interaction, user_id: str):
        ...
        async def premium_role_callback(inter, role, identifier, view):
            ...
            async with session.post(resolve_url, json=packet.to_dict(), headers=headers) as resp:
                    if resp.status in (200, 201):
                        res = await resp.json()
                        embed = await do_allow(inter, res['role'], res['canonical_id'])
        ...
        async def do_allow(inter, role, canonical_id):
            ...
            packet = BotPacketFactory.create_packet(
                packet_type="guild_allow_execution",
                data={
            ...
            async with session.post(url, json=packet.to_dict(), headers=headers) as resp:
                    if resp.status in (200, 201):
                        ...
                        # Fire-and-forget analytics event for allow action
                        ...
                        return success_embed(message=data.get('message', 'Execution has been allowed.'))
                    elif resp.status == 429:
                        err = await resp.json()
                        return throttled_embed(err.get('retry_after', 10))
                    else:
                        err = await resp.json()
                        return error_embed(message=err.get('error', 'Could not allow this user.'))
        ...
        await validate_and_respond(interaction, allow_callback)


async def setup(bot):
    await bot.add_cog(AllowExecution(bot))
